...
async def create_factory_reset_protection(payment: FactoryResetProtectionCreate):
    return await factory_reset_protection_service.create_factory_reset_protection(
        payment
    )
...
async def get_factory_reset_protection(factory_reset_protection_id: UUID):
    payment = await factory_reset_protection_service.get_factory_reset_protection(
...
async def update_factory_reset_protection(
    factory_reset_protection_id: UUID, factory: FactoryResetProtectionUpdate
):
    updated = await factory_reset_protection_service.update_factory_reset_protection(
        factory_reset_protection_id, factory
    )
    if not updated:
...
async def delete_payment(factory_reset_protection_id: UUID):
    deleted = await factory_reset_protection_service.delete_factory_protection(
        factory_reset_protection_id
    )
    if not deleted:
        raise HTTPException(status_code=404, detail="Payment not found")
