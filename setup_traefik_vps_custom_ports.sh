...
if [ $(docker ps | grep -c traefik) -eq 1 ]; then
    ...
    echo -e "${GREEN}Dashboard disponible en: http://traefik.smartpay-oficial.com:${DASHBOARD_PORT}${NC}"
    ...
    echo "- HTTP: ${HTTP_PORT} (en lugar de 80)"
    echo "- HTTPS: ${HTTPS_PORT} (en lugar de 443)"
    echo "- Dashboard: ${DASHBOARD_PORT} (en lugar de 8080)"
else
...
cat > /home/smartpayvps/update_deployment_service.sh << EOF
#!/bin/bash
...
cat > /tmp/deployment_traefik_update.py << 'EOL'
import os
import sys
...
echo "Debido a que Traefik está usando puertos no estándar (${HTTP_PORT} y ${HTTPS_PORT}),"
